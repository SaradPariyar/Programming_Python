...
number = input("Enter any number (negative number to exit): ")
numInch = float(number)
while numInch >= 0:
    numCm = numInch * 2.54
    print(number, "inch(es) =", numCm, "cm")
    number = input("Input any number (negative number to exit): ")
    numInch = float(number)
...
count = 0
inside_Points = 0
total_Points = int(input("How many points to generate for calculating pi value: "))
while count < total_Points:
    ...
    if x*x + y*y < 1:
        inside_Points += 1
    count += 1
print("Value of pi is: ", 4*inside_Points/total_Points)